...
* Example folder: Application example of CP-CHARM. The folder contains:
			...
			- "DefaultOUT_CHARM-like_training_data.csv" and "DefaultOUT_CHARM-like_training_labels.csv", the feature vectors and corresponding labels for all images extracted using the "[TrainTestMode]CHARM-like.cppipe" pipeline
...
1) Download compiled version of CellProfiler
	* Note that pipelines have been designed with CellProfiler release version 2.1.0, which can be downloaded at http://www.cellprofiler.org/previousReleases.shtml
	* We are currently updating the pipelines to CellProfiler's latest release, but in the meantime we recommend only using them with version 2.1.0
...
1) Extracting features in CellProfiler
	* Two pipelines are available, located in the Pipelines folder:
		- "[TrainTestMode]CHARM-like.cppipe" gathers metadata on filename (image ID) and on folder name (class) and outputs two .csv files ("DefaultOUT_CHARM-like_training_data.csv" and "DefaultOUT_CHARM-like_training_labels.csv")
		- "[ClassifyMode]CHARM-like.cppipe" gathers metadata on filename (image ID) only and outputs one .csv file ("DefaultOUT_CHARM-like_data.csv")

	* For either pipeline, either open CP, load the "CHARM-like.cppipe" pipeline you want and run analysis, or run CP headless (from CellProfiler_Head directory) using the following command:

	python CellProfiler.py -p ../CHARM-like.cppipe -c -r -o ../ -i ../Input_Images --plugins-directory=../Modules/
