...
def get_confusion_matrix(classifier_type, validation_type, ckdata, headers, keysize, K=None, exclude_subkey=None, include_subkey=None):
   '''   
   ...
   confusion = np.zeros((numclasses,numclasses), dtype=np.int)
      ...
      ##print 'PCA...'
      #X=np.array(ckdata[:,2:], dtype='float')
      #X=np.array(X[:,np.std(X,0)!=0.0], dtype='float')
      #scaler=Scaler()
      #Xnorm=scaler.fit_transform(X)
      
      ##print Xnorm
      ##print np.sum(np.isnan(np.array(Xnorm)))
      ##print np.where(np.isnan(np.array(Xnorm)))
      ##print Xnorm[:,302]
      
      ##ckdata=np.hstack((np.array(ckdata[:,:2], dtype='S100'), Xnorm))
      #pca = PCA(n_components=0.98)
      #pca_data=pca.fit_transform(Xnorm) 
      ##print pca_data.shape
      ##stop
      #ckdata=np.hstack((np.array(ckdata[:,:2], dtype='S100'), pca_data))
      ##print 'PCA done.'      
   ...
   for training, test in cross_validation_iterator(ckdata, K):
      ...
      if classifier_type=='wnd':
         ...
         meaningful_feats=np.array([])
         first_i=True
         for i in range(0, len(classifier.weight)):
            if classifier.weight[i]!=0:
               pair=np.array([headers[i], classifier.weight[i]])
               if first_i:
                  meaningful_feats=np.append(meaningful_feats, pair)
                  first_i=False
               else:            
                  meaningful_feats=np.vstack((meaningful_feats, pair))
      
                  meaningful_feats=meaningful_feats[meaningful_feats[:,1].argsort(),]
         # Uncomment following line to have a list of the features used for classification
         #print 'Features used for classification: '
         #print meaningful_feats
         ...
         #for i in range(0, len(pred_int)):
            #pred_labels=np.append(pred_labels, [key for key, value in int_labels.iteritems() if value == pred_int[i]][0])
         ##print 'Testing done.'
   ...
   if classifier_type=='wnd':
      return classifier, classes, confusion, percent, avg, avgTotal, meaningful_feats
   else:
      return classifier, classes, confusion, percent, avg, avgTotal
...
def cross_validation(classifier_type, validation_type, labels_csvfile, profile_csvfile, K=None, displayresult=True, exclude_subkey=None, include_subkey=None, standardize=False):
   '''
   ...
   if classifier_type=='wnd': 
      classifier, classes, confusion, percent, avg, avgTotal, meaningful_feats = get_confusion_matrix(classifier_type, validation_type, ckdata, headers, keysize, K, exclude_subkey, include_subkey)
   else:
      classifier, classes, confusion, percent, avg, avgTotal = get_confusion_matrix(classifier_type, validation_type, ckdata, headers, keysize, K, exclude_subkey, include_subkey)      
   ...
   if classifier_type=='wnd': 
      return classifier, avgTotal, meaningful_feats
   else:
